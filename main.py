...
# 초기화면을 출력하여 사용자가 cnv,pp동작 중 선택할 수 있도록 함
op_type=""
...
#skeleton extractio할 준비
mp_hands = mp.solutions.hands
hands = mp_hands.Hands(static_image_mode=True, max_num_hands=1, min_detection_confidence=0.3)
mp_drawing = mp.solutions.drawing_utils

#skeleton을 추출할 이미지와 object detection으로 나온 라벨 파일을 리스트로 저장
...
for i in range(len(label_list)):
    ...
    if(exist_1!=""):
        ...
        y1=int(line[3])
        y2=int(line[4])
        #object detection결과에 따라 이미지 자르기
    ...
    #왼손 오른손으로 나눠 skeleton extraction
...
#skeleton extraction한 결과를 왼손 오른손으로 나눠 csv 파일에 저장
...
#분류모델 함수를 실행하는데 왼손,오른손으로 나눠 실행함
if(op_type=="pp"):
